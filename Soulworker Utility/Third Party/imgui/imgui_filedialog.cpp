...
bool ImGui::FileDialog(bool* open, ImFileDialogInfo* dialogInfo)
{
	...
	if (ImGui::Begin(dialogInfo->title.c_str(), open))
	{
		...
		// Draw path
		ImGui::Text("%s: %s", LANGMANAGER.GetText("STR_FILEDIALOG_PATH").data(), dialogInfo->directoryPath.string().c_str());
		...
		// File Columns
		sprintf_s(label, "%s###Name", LANGMANAGER.GetText("STR_FILEDIALOG_NAME").data());
		...
		ImGui::NextColumn();
		sprintf_s(label, "%s###Size", LANGMANAGER.GetText("STR_FILEDIALOG_SIZE").data());
		if (ImGui::Selectable(label))
		{
		...
		ImGui::NextColumn();
		sprintf_s(label, "%s###Type", LANGMANAGER.GetText("STR_FILEDIALOG_TYPE").data());
		if (ImGui::Selectable(label))
		{
			fileNameSortOrder = ImGuiFileDialogSortOrder_None;
			dateSortOrder = ImGuiFileDialogSortOrder_None;
		...
		ImGui::NextColumn();
		sprintf_s(label, "%s###Date", LANGMANAGER.GetText("STR_FILEDIALOG_DATE").data());
		if (ImGui::Selectable(label))
		{
			fileNameSortOrder = ImGuiFileDialogSortOrder_None;
			sizeSortOrder = ImGuiFileDialogSortOrder_None;
		...
		if (dialogInfo->directoryPath.has_parent_path())
		{
			...
			ImGui::NextColumn();
			sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_TYPE_PARENT").data());
		...
		for (size_t i = 0; i < directories->size(); ++i)
		{
			...
			ImGui::TextUnformatted("-");
			ImGui::NextColumn();
			sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_TYPE_DIR").data());
			ImGui::TextUnformatted(label);
			ImGui::NextColumn();
		...
		sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_NAME").data());
		...
		{
			sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_OPEN").data());
		...
		{
			sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_SAVE").data());
			if (ImGui::Button(label))
			{
				dialogInfo->resultPath = dialogInfo->directoryPath / dialogInfo->fileName;
		...
		sprintf_s(label, "%s", LANGMANAGER.GetText("STR_FILEDIALOG_CANCEL").data());
		if (ImGui::Button(label))
		{
			fileNameSortOrder = ImGuiFileDialogSortOrder_None;
			sizeSortOrder = ImGuiFileDialogSortOrder_None;
